...
function createSourceRowTree (sourceRow,metadataSourceTable) {
    ...
    rowValue ? retorno.value = rowValue : retorno.value = 0;
...
function inicializarArbol(h,w,source,hst,settings,idDiv){
    ...
    try { enableLegend = settings.legend.enableLegend; } catch(e) {}
    var legendPosition = "top-right";
    try { legendPosition = settings.legend.legendPosition; } catch(e) {}
    var legendFontSize = 12;
    try { legendFontSize = settings.legend.legendFontSize; } catch(e) {}
    var legendItemSpacing = 10;
    try { legendItemSpacing = settings.legend.legendItemSpacing; } catch(e) {}
    ...
    function update(source,hst,selectionMngr,localIdDiv) {
        ...
        updateGraph(nodes,source,hst,selectionMngr,localIdDiv, enableLegend, legendPosition, legendFontSize, legendItemSpacing, w, h);
    ...
    function renderCategoryLegend(svg, categories, colorPalette, settings, w, h) {
        if (!settings.legend.enableLegend) return;
        
        // Extract unique categories (up to 7)
        const uniqueCategories = [];
        if (categories && categories[0] && categories[0].values) {
            categories[0].values.forEach(val => {
                if (uniqueCategories.length < 7 && !uniqueCategories.includes(val)) {
                    uniqueCategories.push(val);
                }
            });
        }
        
        // Create color mapping using existing logic
        const colorMapping = uniqueCategories.map((cat, index) => ({
            category: cat,
            color: colorPalette ? colorPalette.getColorByIndex(index).value : "lightsteelblue"
        }));
        ...
        const pos = settings.legend.legendPosition || "top-right";
        const fontSize = settings.legend.legendFontSize || 12;
        const spacing = settings.legend.legendItemSpacing || 10;
        let xOffset = 10, yOffset = 10;
        if (pos === "top-right") { xOffset = w - 150; }
        else if (pos === "bottom-left") { yOffset = h - (uniqueCategories.length * (fontSize + spacing)); }
        else if (pos === "bottom-right") { xOffset = w - 150; yOffset = h - (uniqueCategories.length * (fontSize + spacing)); }
        
        // Render legend items
        colorMapping.forEach((item, index) => {
            const y = yOffset + index * (fontSize + spacing);
    ...
    function updateGraph(nodes,source,hst,selectionMngr,localIdDiv, enableLegend, legendPosition, legendFontSize, legendItemSpacing, w, h){
        ...
        // Add legend
        renderCategoryLegend(svg, categories, hst.colorPalette, {
            legend: {
